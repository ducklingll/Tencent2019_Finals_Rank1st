...
def construct_train_data():
    ...
    log_imp_dfs=[]
    imp_labels_all=[]
    for file,day in tqdm([('../data/track_log_201904{}.out'.format(day),day) for day in range(10,23)],total=len(range(10,23))):
        if file[-3:]=='out':
            log_df=pd.read_csv(file, sep='\t',names=['request_id','request_timestamp','uid','position','aid_info']).sort_values(by='request_timestamp')
        else:
    ...
    request_df=pd.concat(request_dfs,0)
    ...
    request_df=pd.concat(request_dfs[:-1],0)
    request_df.to_pickle('preprocess_data/request_train_dev.pkl')
    op_df=pd.concat(op_dfs[:-1],0)
    ...
    gc.collect()
    items=[]
    for imp_labels in imp_labels_all:
    ...
    items=[]
    for imp_labels in imp_labels_all[:-1]:
        for key in imp_labels:
            items.append(list(key)+imp_labels[key])
    log_label=pd.DataFrame(items)
    log_label.columns=['aid','day','label_0','label_1']
